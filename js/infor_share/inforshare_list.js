...
app.controller('myCtrl', function($scope,$http) {
    ...
    function myGridjiazai(p){
        if($scope.so_edit_name == undefined){
            ...
            $scope.so_type = ""
        }if($scope.so_filename == undefined) {
            $scope.so_filename = ""
        }
        $http.get(basePath+get_url+"access_token="+localStorage.getItem("token")+
            "&add_user_name^="+$scope.so_edit_name+
            "&infor_title^="+$scope.so_title+
            "&infor_type^="+$scope.so_type+
            "&filename^="+$scope.so_filename+
            "&page_size=15"+
            "&page="+p)
                ...
                if(res.response.success == 1){
                    ...
                    for(var i=0; i < get_data.length;i++){
                        ...
                        myGrid.addRow(str,[
                            ...
                            get_data[i].infor_title,
                            get_data[i].infor_type,
                            get_data[i].filename,
                            get_data[i].add_time,
                ...
                callback:function(result){
                    if(result==true){
                        $http.get(basePath+"api/v1.0/like/islike?access_token="+localStorage.getItem("token")+
                            "&user_id="+localStorage.getItem("mobile")+"&type=inforshare&like_id="+$scope.this_row_id
                        ).success(function(res){
                            if(res.response.islike=="1"){
                                dhx_alert("no repeat like")
                            }else{
                                $http({
                                    method:"put",
    ...
    $("input:text").bind("input propertychange",function(){
        ...
        $scope.so_filename=document.getElementById("parame_d").value;
