...
app.controller('myCtrl', function($scope,$http) {
    ...
    var detail_url = "detail_infor_share.html?";
    ...
    function init(){
        if(localStorage.getItem("token") == undefined ||localStorage.getItem("token") == null){
            ...
            myGrid.setHeader("editor,title,category,add_time,last_edit_time,deatil,nolike");//设置表头
    ...
    function myGridjiazai(){
        $http.get(basePath+get_url+"access_token="+localStorage.getItem("token")+"&user_id="+localStorage.getItem("job_no"))
            .success(function(res){
                if(res.response.success == 1){
                    $scope.location_data = res.response.data.inforshare_like_detail;
